...
rule vectorize_msearch:
    ...
    run:
        ...
        # for msearch we will vectorize with all possible kmers
        #     this may make the vectorized files very large
        kmers = skm.io.read_output_kmers(input.kmers)
            ...
            for seq, sid in zip(seq_list, id_list):
                ...
                results["vector"] += [
                    skm.transform.vectorize_string(
                        ...
                        filter_list=kmers,  # For msearch we're using all kmers
                        verbose=False,  # way too noisy for batch process
                        log_file=log[0],
                    )
                ]
